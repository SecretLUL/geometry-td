  ...
  public override drawPixi(g: PIXI.Graphics, part: "base" | "turret"): void {
    ...
    if (part === "base") {
      ...
      if (this.constructionTimer > 0) {
        ...
        g.circle(0, yOffset, pulseRadius1).stroke({
          color: "#ff4757",
        ...
        if (pulseRadius2 > 0.2 * baseR) {
          g.circle(0, yOffset, pulseRadius2).stroke({
            color: "#ff7675",
            alpha: 0.3 * (1 - pulseRadius2 / baseR),
            width: 1.0,
          });
        }
      }
      ...
      if (this.masteryUnlocked) {
        ...
        for (let i = 0; i < 4; i++) {
          ...
          g.circle(
            Math.cos(ang) * (baseR - 4),
            yOffset + Math.sin(ang) * (baseR - 4),
            2 * scale
          ).fill({ color: dotColor });
        }
      }
    ...
    if (part === "turret") {
      const turretColor1 =
        this.specialization === "nuke"
          ? "#badc58"
          : this.specialization === "cluster"
            ? "#57606f"
            : "#600";
      const turretColor2 =
        this.specialization === "nuke"
      ...
      g.circle(0, yOffset, 10 * scale).stroke({
        color: this.specialization === "cluster" ? "#d63031" : turretColor1,
        width: 1.5,
      });
      ...
      if (this.specialization === "cluster") {
        if (this.masteryUnlocked) {
          ...
          // 1. Pod chassis
          g.roundRect(4 * scale, yOffset - 9 * scale, 14 * scale, 18 * scale, 3 * scale).fill({
            color: "#2f3542",
          });
          g.roundRect(4 * scale, yOffset - 9 * scale, 14 * scale, 18 * scale, 3 * scale).stroke({
            color: "#ffd700",
            width: 1.5,
          }); // Golden trim for mastery
          ...
          g.moveTo(0, yOffset - 8 * scale)
            .lineTo(6 * scale, yOffset - 12 * scale)
            .lineTo(10 * scale, yOffset - 12 * scale)
            .lineTo(6 * scale, yOffset - 8 * scale)
            .closePath()
            .fill({ color: "#d63031" });
          g.moveTo(0, yOffset + 8 * scale)
            .lineTo(6 * scale, yOffset + 12 * scale)
          ...
          g.rect(-10 * scale, yOffset - 4 * scale, 4 * scale, 8 * scale).stroke({
            color: "#ff4757",
            width: 1,
          });
        } else {
          ...
          // Top barrel
          g.roundRect(4 * scale, yOffset - 6 * scale, 10 * scale, 3 * scale, 1 * scale).fill({
            color: "#747d8c",
          });
          ...
          g.roundRect(5 * scale, yOffset - 2 * scale, 11 * scale, 4 * scale, 1 * scale).fill({
            color: "#2f3542",
          });
          g.rect(14 * scale, yOffset - 2 * scale, 3 * scale, 4 * scale).fill({ color: "#ff4757" }); // glowing tip
          // Bottom barrel
          g.roundRect(4 * scale, yOffset + 3 * scale, 10 * scale, 3 * scale, 1 * scale).fill({
            color: "#747d8c",
          });
        ...
        g.rect(0, yOffset - 6 * scale, 12 * scale, 12 * scale).fill({
          color: this.specialization === "nuke" ? "#2d3436" : "#ff4040",
        });
        g.rect(10 * scale, yOffset - 7 * scale, 4 * scale, 14 * scale).fill({
          color: this.specialization === "nuke" ? "#badc58" : "#ffffff",
        });
      }
    }
  }
