...
export class PrismaTower extends Tower {
  ...
  public override initPixi(): void {
    ...
    if (!this.pixiBasePlatformGraphics) {
      ...
      this.pixiSprite.addChildAt(this.pixiBasePlatformGraphics, 0); // behind turret
    }
  ...
  public override updatePixi(): void {
    ...
    if (this.constructionTimer <= 0) {
      ...
      if (this.pixiReactorPoolGraphics && this.masteryUnlocked) {
      ...
      if (this.pixiStandardRingGraphics && !this.masteryUnlocked) {
        ...
        if (this.pixiTurretOrbitGraphics) {
          ...
          for (let i = 0; i < 3; i++) {
            ...
            this.pixiTurretOrbitGraphics
              .moveTo(ox, oy)
              .lineTo(0, 0)
              .stroke({ color: this.currentColor, alpha: 0.25, width: 0.8 * scale });
          }
