/*
 * @file: frontend/src/js/entities/enemies/types/event.ts
...
export class CollectorEnemy extends BaseEnemy {
  constructor(waveNumber: number) {
    ...
    this.reward = Math.floor(
      Config.ENEMY_REWARD_BASE * 180 * Math.pow(Config.ENEMY_REWARD_MULTIPLIER, waveNumber - 1)
    );
    this.initHp();
  }
...
export class FortressEnemy extends BaseEnemy {
  constructor(waveNumber: number) {
    ...
    this.reward = Math.floor(
      Config.ENEMY_REWARD_BASE * 10 * Math.pow(Config.ENEMY_REWARD_MULTIPLIER, waveNumber - 1)
    );
  ...
  public override updatePixi(): void {
    ...
    if (this.shieldActive && this.shieldGraphics) {
      ...
      this.shieldGraphics
        .circle(0, 0, this.radius + 10 + pulse)
        .stroke({ color: 0x74b9ff, width: 4 });
      ...
      this.shieldGraphics
        .rect(-15, -this.radius - 18, 30 * (this.shieldHp! / this.maxShieldHp!), 3)
        .fill({ color: 0x74b9ff });
    }
  }
}
...
export class SplinterEnemy extends BaseEnemy {
  constructor(waveNumber: number) {
    ...
    ); // 7g reward at wave 1, scales up to 10
    this.initHp();
...
export class SplinterFragmentEnemy extends BaseEnemy {
  constructor(waveNumber: number) {
    ...
    this.reward = Math.max(
      1,
      Math.floor(
        Config.ENEMY_REWARD_BASE * 0.4 * Math.pow(Config.ENEMY_REWARD_MULTIPLIER, waveNumber - 1)
      )
    ); // 2g reward
    this.initHp();
  ...
  public override reset(waveNumber: number): void {
    const baseHp = Config.ENEMY_BASE_HP;
    const hpMultiplier = Config.getHpMultiplier(waveNumber);
    this.maxHp = Math.max(1, Math.floor(baseHp * hpMultiplier * 0.2));
    this.reward = Math.max(
      1,
      Math.floor(
        Config.ENEMY_REWARD_BASE * 0.4 * Math.pow(Config.ENEMY_REWARD_MULTIPLIER, waveNumber - 1)
