/*
 * @file: frontend\src\js\entities\enemies\types\bosses.ts
...
export class BossEnemy extends BaseEnemy {
    ...
    public override update(): 'stunned' | 'reached_end' | 'moving' {
        ...
        if (!state.isHost) {
            ...
            if (hpFill) {
                ...
                if (this.hp < this.maxHp * 0.25) {
                    ...
                    hpFill.style.background = isFlickering ? '#ffffff' : 'linear-gradient(90deg, #8b0000, #ff0000)';
                    if (bossHpContainer) bossHpContainer.style.borderColor = isFlickering ? '#ffffff' : '#ff0000';
        ...
        this.abilityTimer!++;
        if ((this.nextAbility === 'spawn' && this.abilityTimer! >= 480) ||
    ...
    public override updatePixi(): void {
        ...
        if (isEnraged && !state.isPaused) {
            if (Math.random() < 0.3) {
                PoolManager.getParticle(this.x + (Math.random() - 0.5) * this.radius, this.y + (Math.random() - 0.5) * this.radius, '#555', 2, Math.random() * 4 + 2);
            }
        }
        ...
        g.ellipse(0, 0, this.radius * 0.15 * pulse, this.radius * 0.35 * pulse).fill({ color: '#000000' });
    }
}
...
export class DefragmenterEnemy extends BaseEnemy {
    constructor(waveNumber: number) {
        ...
        const baseHp = Config.ENEMY_BASE_HP;
        const hpMultiplier = Config.getHpMultiplier(waveNumber);
        this.maxHp = Math.floor(baseHp * hpMultiplier * 100);
        this.reward = Config.WAVE_BONUS_BASE * 10;
        this.initHp();
...
export class DefragmenterFragmentEnemy extends BaseEnemy {
    constructor(waveNumber: number) {
        ...
        const baseHp = Config.ENEMY_BASE_HP;
        const hpMultiplier = Config.getHpMultiplier(waveNumber);
        this.maxHp = Math.floor(baseHp * hpMultiplier * 30);
        this.reward = Math.floor((Config.ENEMY_REWARD_BASE * 4) * Math.pow(Config.ENEMY_REWARD_MULTIPLIER, waveNumber - 1));
        this.initHp();
    ...
    public override drawShape(g: any): void {
        ...
        g.lineTo(Math.cos(-Math.PI/2)*this.radius*0.5, Math.sin(-Math.PI/2)*this.radius*0.5);
