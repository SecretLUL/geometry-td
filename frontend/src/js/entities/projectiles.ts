...
export class Projectile {
  ...
  public update(): void {
    ...
    if (this.active) {
      ...
      if (
        !this.targetPoint &&
        (!this.target || (this.target as Enemy).hp <= 0 || (this.target as Enemy).deadMarked)
      ) {
          ...
          if (this.aoeRadius > 0) {
              ...
              if (this.isCluster) {
                ...
                for (let i = 0; i < clusterCount; i++) {
                  ...
                  if (miniEnemy) {
                    ...
                    const miniProj = PoolManager.getProjectile(
                      mx,
                      my,
                      miniEnemy,
                      this.damage * 0.4,
            ...
            if (canSpawnParticles) {
              const explosionColor =
                this.tower && this.tower.type === "Sniper" ? "#a0d8ef" : "#fca311";
            ...
            if (state.isHost && this.target) {
              ...
              if (this.bounceCount > 0) {
                ...
                if (nextTarget) {
                  const nextProj = PoolManager.getProjectile(
                    this.x,
                    this.y,
                    nextTarget,
                    this.damage,
