...
export function executeStartWave(data: any): void {
  ...
  state.enemiesToSpawn =
    Config.WAVE_BASE_ENEMIES + Math.floor(state.wave * Config.WAVE_ENEMIES_MULTIPLIER);
  ...
  if (newlyUnlocked.length > 0) {
    const enemyNames = newlyUnlocked
      .map((type) => {
...
export function handleWaveLogic(): void {
  ...
  if (state.enemiesToSpawn > 0) {
    if (state.spawnCooldown <= 0) {
      ...
      if (type === "Swarm") {
        ...
        if (type === "Accelerator") {
          ...
          state.activeAccelerators.push(newEnemy);
        }
      }
    ...
    createCoinBurst(
      (app.canvas.clientWidth || app.canvas.width) / 2,
      (app.canvas.clientHeight || app.canvas.height) / 2,
      20
    );
    updateUI();
