 ...
 * @purpose: Highly optimized object pooling system for reusing visual elements (particles,
 *           projectiles, floating text, shockwaves). Includes logic to merge nearby gold pop-ups.
...
function parseGoldText(text: string): { amount: number; suffix: string } | null {
  const match = text.match(/^\+(\d+)g(?:\s+(Bounty|Bank))?$/i);
...
export const PoolManager = {
  ...
  init(): void {
    ...
    this.groundEffectsList = [...this.radiationAreas.getArray(), ...this.shockwaves.getArray()];
  },
  ...
  getProjectile(
    ...
    return this.projectiles
      .get()
      .init(x, y, target, damage, tower, aoeRadius, speed, bounceCount, isCluster);
  },
        ...
        if (ft.active && ft.isGold) {
          ...
          if (distSq <= radiusSq) {
            ...
            if (currentParsed) {
              ...
              let suffix = "";
              if (currentParsed.suffix === "Bounty" || newParsed.suffix === "Bounty") {
                suffix = " Bounty";
              } else if (currentParsed.suffix === "Bank" || newParsed.suffix === "Bank") {
                suffix = " Bank";
              }
              ...
              // Call the overhauled merge method on the FloatingText class
              ft.merge(totalAmount, suffix, x, y);
