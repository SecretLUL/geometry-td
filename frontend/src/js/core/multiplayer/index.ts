...
Multiplayer.init = function(startWaveCallback: (data?: any) => void, updateUICallback: () => void): void {
    ...
    try {
        ...
        const isDevEnv = window.location.hostname.includes('gtd-dev') || 
    ...
    const runSyncLoop = () => {
        if (state.isHost && !state.isPaused && !state.gameOver) {
            ...
            const isIdle = state.enemies.length === 0 && !state.isWaveActive && (!state.projectileEvents || state.projectileEvents.length === 0);
            let skipSync = false;
    ...
    if (isDevEnv && counterEl) {
        ...
        if (!toggleBtn) {
            ...
            toggleBtn.addEventListener('mouseenter', () => {
                if (toggleBtn) {
                    toggleBtn.style.background = 'linear-gradient(135deg, rgba(252, 163, 17, 0.4), rgba(212, 154, 0, 0.2))';
            ...
            toggleBtn.addEventListener('mouseleave', () => {
                if (toggleBtn) {
                    toggleBtn.style.background = 'linear-gradient(135deg, rgba(252, 163, 17, 0.2), rgba(212, 154, 0, 0.1))';
