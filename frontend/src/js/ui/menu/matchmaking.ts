  ...
  private initModeModal(): void {
    ...
    startSolo?.addEventListener("click", () => {
      ...
      transitionAndRedirect(
        `game.html?map=${encodeURIComponent(this.selectedMapName)}&mode=singleplayer`
      );
    });
    ...
    startPublic?.addEventListener("click", () => {
      if (!this.selectedMapName) return;
      transitionAndRedirect(
        `game.html?map=${encodeURIComponent(this.selectedMapName)}&mode=public`
      );
    });
    ...
    createPrivate?.addEventListener("click", () => {
      if (!this.selectedMapName) return;
      transitionAndRedirect(
        `game.html?map=${encodeURIComponent(this.selectedMapName)}&mode=private&action=create`
      );
    });
