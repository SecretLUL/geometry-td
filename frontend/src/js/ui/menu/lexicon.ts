...
export class LexiconController {
  ...
  public initLexicon(): void {
    ...
    categories.forEach((cat) => {
      ...
      const mainEnemies = catEnemies.filter(
        (k) => k !== "DefragmenterFragment" && k !== "DefragmenterSubfragment"
      );

      mainEnemies.forEach((key) => {
        const data = EnemyData[key];
        const isDiscovered =
          !!discovered[key] || (!!data.unlockWave && recordWave >= data.unlockWave);
        ...
        btn.addEventListener("click", () => {
          ...
          btn.style.background = isDiscovered
            ? "rgba(76, 201, 240, 0.1)"
        ...
        if (key === "Defragmenter") {
          ...
          fragments.forEach((fragKey) => {
            ...
            const fragDiscovered =
              !!discovered[fragKey] || (!!fragData.unlockWave && recordWave >= fragData.unlockWave);
            ...
            fragBtn.addEventListener("click", () => {
              ...
              fragBtn.style.background = fragDiscovered
                ? "rgba(76, 201, 240, 0.1)"
                : "rgba(255,255,255,0.05)";
              fragBtn.style.borderColor = fragDiscovered
                ? "var(--tesla-cyan)"
  ...
  private updateLexiconDetails(key: string, data: any, isDiscovered: boolean): void {
    ...
    if (isDiscovered) {
      ...
      desc.innerText =
        "Tactic data restricted. Neutralize entity in simulation to unlock database entry.";
      hpBar.style.width = "0%";
    ...
    const initLexiconApp = async () => {
      ...
      const tickFn = () => {
        ...
        if (enemyType !== "Accelerator" && enemy.rotation !== undefined) {
          enemy.rotation +=
            0.02 * Math.max(1, enemy.speed) * (enemy.rotationSpeedMultiplier ?? 1.0);
        }
        if (enemy.outerRotation !== undefined)
          enemy.outerRotation += 0.02 * Math.max(1, enemy.speed);
