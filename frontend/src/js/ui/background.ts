  ...
  private animate(): void {
    ...
    if (isMenu) {
      ...
      for (let l = 0; l < 2; l++) {
        ...
        this.laserGraphics
          .circle(headPt.x, headPt.y, 14 + pulse)
          .fill({ color: laserColor, alpha: 0.15 });
        this.laserGraphics
          .circle(headPt.x, headPt.y, 8 + pulse * 0.5)
          .fill({ color: laserColor, alpha: 0.5 });
      ...
      for (let i = this.laserSparks.length - 1; i >= 0; i--) {
        ...
        this.laserGraphics
          .circle(spark.x, spark.y, spark.size * spark.life)
          .fill({ color: spark.color, alpha: currentAlpha });
      }
