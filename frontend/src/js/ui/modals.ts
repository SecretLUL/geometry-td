...
export function showGameOverScreen(): void {
  ...
  if (goGoldRest)
    goGoldRest.innerText = formatNumber(state.totalGoldEarned - state.totalGoldFromInterest);
...
export function updateContextShopPosition(): void {
  ...
  if (
    ...
    !state.contextShopCell ||
    !state.contextShopPos
  )
    return;
...
export function hideContextShop(): void {
  ...
  if (
    !contextShop ||
    contextShop.classList.contains("hidden") ||
    contextShop.classList.contains("closing")
  ) {
...
export function showContextShop(x: number, y: number, col: number, row: number): void {
  ...
  const shopItems = [
    ...
    { type: "Generator", cost: TowerData["Generator"].baseCost },
  ];
